...
from apps.accounts.utils import record_audit_log, get_request_ip
...
class AnnouncementListView(JWTAuthMixin, JsonBodyMixin):
    """公告列表（管理员）"""
    ...
    def get(self, request):
        ...
        result = paginator.paginate(queryset, page, page_size)
...
@method_decorator(csrf_exempt, name="dispatch")
class AnnouncementCreateView(JWTAuthMixin, AdminRequiredMixin, JsonBodyMixin):
    """创建公告"""
...
@method_decorator(csrf_exempt, name="dispatch")
class AnnouncementDetailView(JWTAuthMixin, JsonBodyMixin):
    """公告详情"""
...
@method_decorator(csrf_exempt, name="dispatch")
class AnnouncementUpdateView(JWTAuthMixin, AdminRequiredMixin, JsonBodyMixin):
    """更新公告"""
...
@method_decorator(csrf_exempt, name="dispatch")
class AnnouncementDeleteView(JWTAuthMixin, AdminRequiredMixin, JsonBodyMixin):
    """删除公告"""
...
@method_decorator(csrf_exempt, name="dispatch")
class MyAnnouncementsView(JWTAuthMixin):
    """我的公告（当前用户可见的公告）"""
...
@method_decorator(csrf_exempt, name="dispatch")
class MarkAsReadView(JWTAuthMixin):
    """标记公告已读"""
...
@method_decorator(csrf_exempt, name="dispatch")
class UnreadCountView(JWTAuthMixin):
    """未读公告数量"""
